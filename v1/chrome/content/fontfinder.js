...
(function () {
    ...
    if (manager.getBoolPref("extensions.fontfinder@bendodson.com.webExtension")) {
        var v50 = Components.classes["@mozilla.org/xpcom/version-comparator;1"]
        .getService(Components.interfaces.nsIVersionComparator)
        .compare(version, '50.0');
        console.error(v50, getBrowser().addTab)
        if (v50 >= 0) {
            window.setTimeout(function () {
                try {
                    var require = Components.utils.import('resource://gre/modules/commonjs/toolkit/require.js', {}).require;
                    var tabs = require('sdk/tabs');
                    tabs.open("https://addons.mozilla.org/en-US/firefox/addon/font-inspect/");
                    var promptService = Components.classes['@mozilla.org/embedcomp/prompt-service;1']
                              .getService(Components.interfaces.nsIPromptService);
                   promptService.alert(null, "Font Finder Migration", "Font Finder is moving to WebExtensions!\n\nPlease give the WebExtension version a try and replace it with this XUL version.");
                   manager.setBoolPref("extensions.fontfinder@bendodson.com.webExtension", false);
                }
                catch (e) {}
            }, 5000);
        }
...
(function () {
/*    var require, tabs, getTabForId, getTabContentWindow;
    try {
        require = Components.utils.import("resource://gre/modules/commonjs/toolkit/require.js").require;
        tabs = require("sdk/tabs");
        getTabContentWindow = require ("sdk/tabs/utils").getTabContentWindow;
        getTabForId = require ("sdk/tabs/utils").getTabForId;
    }
    catch (e) {}
*/
    function currentWindow () {
/*        if (tabs && getTabContentWindow) {
            return getTabContentWindow (getTabForId(tabs.activeTab.id));
        }*/
    ...
    this.getStyle = function (element, style) {
        ...
        try {
            ...
            try {
                console.error(2);
                return currentWindow().getComputedStyle(element, null).getPropertyValue(style);
            }
            catch (e) {
                try {
                return document.commandDispatcher.focusedWindow.getComputedStyle(element, null).getPropertyValue(style);
            console.error(3);
                }
                catch(e) {
            console.error(4);
                    return element.ownerDocument.defaultView.getComputedStyle(element, null).getPropertyValue(style);
                }
            }
        }
    };
    ...
    this.loadDetective = function () {
        if (fontFinder.detective == false || typeof fontFinder.detective.curWin != "undefined") {
